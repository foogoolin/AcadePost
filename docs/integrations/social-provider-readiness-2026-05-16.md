...
- The Docker runtime receives the provider credentials from `.env`.
...
| Facebook Page `facebook` | Yes | Yes | Yes | Needs platform review | Requires Meta app, Pages permissions, page task rights, and likely app review for public use. |
| Instagram FB-linked `instagram` | Yes | Yes | Yes | Needs platform review | Requires Instagram professional account linked to a Facebook Page and `instagram_content_publish`. |
...
| Threads `threads` | Yes | Yes | Yes | Needs platform review | Requires `threads_basic` and `threads_content_publish`. HTTPS callback is strongly preferred. |
...
| YouTube `youtube` | Yes | Yes | Yes | Needs platform review | Uses YouTube Data API upload scope. Google public apps using sensitive user-data scopes may need verification. |
...
| Telegram `telegram` | Yes | Yes | Yes, added in this pass | Easy first test | Needs bot token and bot added to target chat/channel. Removed chat object logging during this pass. |
...
| Pinterest `pinterest` | Yes | Yes | Yes | Needs platform review | Needs OAuth app and `boards:*` / `pins:*` scopes. Pinterest docs require access token scopes for board/pin management. |
