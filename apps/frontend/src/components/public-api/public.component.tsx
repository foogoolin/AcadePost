...
const getMcpConfig = (
  ...
  if (method === 'path') {
    switch (client) {
      case 'Claude Code':
        return {
          config: `claude mcp add acadepost --transport http "${urlWithKey}"`,
          hint: 'Run this command in your terminal.',
        };
      case 'Cursor':
        return {
          config: json({ mcpServers: { acadepost: { url: urlWithKey } } }),
          hint: 'Add to .cursor/mcp.json in your project root.',
        };
      case 'VS Code / Copilot':
        return {
          config: json({
            servers: { acadepost: { type: 'http', url: urlWithKey } },
          }),
      ...
      case 'Windsurf':
        return {
          config: json({
            mcpServers: { acadepost: { serverUrl: urlWithKey } },
          }),
          hint: 'Add to ~/.codeium/windsurf/mcp_config.json',
        };
      case 'Amp':
        return {
          config: `amp mcp add acadepost ${urlWithKey}`,
          hint: 'Run this command in your terminal.',
        };
      case 'Codex':
        return {
          config: `# ~/.codex/config.toml\n\n[mcp_servers.acadepost]\nurl = "${urlWithKey}"`,
      ...
      case 'Warp':
        return {
          config: json({ acadepost: { url: urlWithKey } }),
  ...
  switch (client) {
    case 'Claude Code':
      return {
        config: `claude mcp add acadepost \\\n  --transport http \\\n  --header "Authorization: ${bearer}" \\\n  "${urlBase}"`,
        hint: 'Run this command in your terminal.',
      };
    case 'Cursor':
      return {
        config: json({
          mcpServers: {
            acadepost: { url: urlBase, headers: { Authorization: bearer } },
          },
        }),
        hint: 'Add to .cursor/mcp.json in your project root.',
      };
...
const localCliSteps = [
  {
    label: 'Prepare an API client',
    code: 'Use your AcadéPost API key with your own automation client.',
  },
  {
    label: 'Configure MCP',
    code: 'Use the MCP configuration section below.',
  },
  {
    label: 'Schedule content',
    code: 'Create posts through the API or the existing publishing calendar.',
  },
] as const;
...
const ciCliSteps = [
  {
    label: 'Prepare an API client',
    code: 'Use your AcadéPost API key with your CI automation.',
  },
  {
    label: 'Set your API key as an environment variable',
    code: 'export ACADEPOST_API_KEY="{API_KEY}"',
  },
  {
    label: 'Call the API',
    code: 'Send authorized requests to your AcadéPost backend.',
  },
] as const;
      ...
      <div className="bg-newBgColorInner px-[20px] py-[14px] border-b border-newBorder flex items-start justify-between gap-[12px]">
        <div>
          ...
          <div className="text-[13px] text-customColor18 mt-[2px]">
            {t(
              'cli_description',
              'Use the API key and MCP endpoint to automate scheduling from your own tools.'
            )}
          </div>
        </div>
      </div>
  ...
  return (
    <div className="flex flex-col gap-[40px]">
      <div className="text-[14px] text-textColor leading-[1.7]">
        ...
        {t(
          'api_auth_note_line2',
          'If you are building a product that schedules posts on behalf of other AcadéPost users,'
        )}
        <br />
        {t(
          'api_auth_note_line3',
